...
**Button Handling**:
*   **Use Shared `Button` Class**: From `bumblebee_hardware.py`. It handles debouncing (`debounce_delay`) and rising/falling edge interrupts perfectly.
*   **Long Press (Reset)**:
...
*   **Use Shared `Knob` Class**: From `bumblebee_hardware.py`. Use `.percent()` or `.range(steps)` to get clean values without jitter.
